...
describe('selection translate store', () => {
  ...
  it('opens with the preferred Arabic target and uses a tool-free oneshot outside the chat session', async () => {
  ...
  it('keeps a Stop control globally reachable for selection speech without a composer', async () => {
    await import('@/app/chat/composer/voice-activity')
    ...
    try {
      const mounted = render(host?.render?.() ?? null)
